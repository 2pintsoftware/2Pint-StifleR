...
If(($Uninstall -eq $false) -and (!$Defaults)){Write-Error "No Default .ini file specified - exiting"
Exit 1
}
If($Defaults){$FileContent = Get-IniContent $Defaults
...
try
{
    ...
    $MSIInProgress = $True
    $LoopCounter++
    If($loopcounter -eq 120){write-warning "Timeout waiting for MSI Mutex - Exiting"
    Exit 1}
}
catch
{
...
If (!$evt){Write-Error "StifleR Service Stop Timed out - Exiting"
$(TimeStamp) + "StifleR Service Stop Timed out - Exiting" | Out-File -FilePath $Logfile -Append -Encoding ascii
Exit 1
          }
}
Else {Write-Error "StifleR Service Stop Timed out - Exiting"
$(TimeStamp) + "StifleR Service Stop Timed out - Exiting" | Out-File -FilePath $Logfile -Append -Encoding ascii
Exit 1
}
...
Else {Write-Debug "StifleR Service not found, possible new install"
...
$log = try { Get-WinEvent -Log $EventLogName -ErrorAction Stop
    }
catch [Exception] {
...
If($Uninstall -eq $true){Write-Debug "Uninstall Complete - exiting"
...
$key="Features"
$v = (Get-Appsetting $StifleRConfig | where {$_.key -eq $key}).value #return the current value of features
$a = $v + "," + $Features
...
$node2 = $xml.Configuration.appsettings.add | where {$_.key -eq "EnableDebugLog"}
$node2.Value = "6"
$node3 = $xml.Configuration.appsettings.add | where {$_.key -eq "EnableDebugTelemetry"}
$node3.Value = "1"
$node4 = $xml.Configuration.appsettings.add | where {$_.key -eq "SignalRLogging"}
$node4.Value = "1"
...
Else {write-warning "StifleR Service Start Timed out - Retry"
